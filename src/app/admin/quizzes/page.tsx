...
export default function AdminQuizzesPage() {
  ...
  const handleSaveQuiz = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/courses/${quizCourseId}/quizzes`, {
        method: "POST",
  ...
  return (
    <div className="space-y-8" dir="rtl">
            ...
            >
              ...
              <CardContent className="p-5 pt-0 space-y-4">
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-[11px] text-slate-400">مؤقت خادم موثق</span>
                  <Link href={`/admin/courses/${quiz.courseId}`} className="text-xs font-bold text-blue-600 hover:underline">
                    تعديل في المنهج ←
                  </Link>
                </div>
              </CardContent>
      ...
      <Dialog open={quizModalOpen} onOpenChange={setQuizModalOpen}>
        <DialogHeader>
          <DialogTitle>إنشاء اختبار تقييمي جديد</DialogTitle>
          <DialogDescription>
