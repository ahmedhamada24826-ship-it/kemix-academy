...
import { GradeTaskSubmissionSchema } from "@/server/domain/tasks/task-schemas";
...
export async function POST(
  ...
  try {
    ...
    const validation = GradeTaskSubmissionSchema.safeParse(body);

    if (!validation.success) {
      return apiError(
        "VALIDATION_ERROR",
        validation.error.errors[0]?.message || "Invalid grade data",
        422,
        validation.error.flatten()
      );
    }

    const submission = await taskService.gradeSubmission(submissionId, user, validation.data);
    ...
    return apiError("TASK_GRADE_ERROR", message, status);
  }
}
